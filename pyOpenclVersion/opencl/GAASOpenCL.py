...
class Gaas_OCL_API:
    def __init__(self, oclDevice=None) -> None:
        ...
        if oclDevice == None:
            ...
            if pyopenCLDeviceType == "CPU":
                self.dev = platform[pyopenCLDevice].get_devices(
    ...
    def runVoigtTest(self, wvn: np.array):
        ...
        wvn_g = cl.Buffer(
            self.ctx, self.mf.READ_ONLY | self.mf.COPY_HOST_PTR, hostbuf=wvn
        )
    ...
    def voigtSim(
        ...
        db_g = cl.Buffer(
            self.ctx,
            self.mf.READ_ONLY | self.mf.COPY_HOST_PTR,
            featureDatabase.nbytes,
            hostbuf=featureDatabase,
        )  # feature DB
    ...
    def voigtSim_raw(
        ...
        magic_numbers = [
            ...
            32000,
            64000,
        ]  # for some reason, running in batches of this many lines at a time is orders of magnitude faster
        magic_num = 2000
